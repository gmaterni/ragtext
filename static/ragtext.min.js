...
adapter:PROVIDER_CONFIG$$module$static$js$llm_provider[a].adapter};UaDb$$module$static$js$services$uadb.saveJson(DATA_KEYS$$module$static$js$services$data_keys.KEY_PROVIDER,this.config);this._updateActiveModelDisplay();this.isTreeVisible&&this._buildTreeView();this.toggleTreeView()},_updateActiveModelDisplay(){const a=document.getElementById("active-model-display");a&&(a.textContent=`${this.config.provider} / ${this.config.model} (${this.config.windowSize}k)`)},showConfig(){const a=LlmProvider$$module$static$js$llm_provider.getConfig(),
...
alert(`requestGet()\nurl: ${a}\n${b}`),c;}}var module$static$js$services$http_request={};module$static$js$services$http_request.requestGet=requestGet$$module$static$js$services$http_request;const Spinner$$module$static$js$app_ui={show:()=>{document.querySelector("#id-text-out .div-text").classList.add("spinner-bg");const a=document.getElementById("spinner");a.classList.add("show-spinner");a.addEventListener("click",Spinner$$module$static$js$app_ui.stop)},hide:()=>{document.querySelector("#id-text-out .div-text").classList.remove("spinner-bg");const a=document.getElementById("spinner");a.classList.remove("show-spinner");a.removeEventListener("click",Spinner$$module$static$js$app_ui.stop)},
...
a.key||a.shiftKey||(a.preventDefault(),this.send2())},async send0(){if(0==DocsMgr$$module$static$js$services$docs_mgr.names().length)alert("Non vi sono documenti da elaborare.\n  Se vuoi iniziare una conversazione usa il pulsante verde o return  ");else if(await confirm("Confermi creazioene Base di conoscenza ?")){setResponseHtml$$module$static$js$app_ui("");Spinner$$module$static$js$app_ui.show();try{AppMgr$$module$static$js$app_mgr.initApp(),await ragEngine$$module$static$js$rag_engine.buildKnBase()&&
alert(" Base di conoscenza creata con successo!")}catch(a){if(console.error("Error send0",a),a&&"CancellationError"===a.type&&499===a.code)alert("Richiesta interrotta");else{let b=errorDumps$$module$static$js$app_ui(a);alert(b)}}finally{Spinner$$module$static$js$app_ui.hide()}}},async send1(){const a=this.inp.value;if(!a)alert("Ricorda di scrivere la Query  ");else if(!await idbMgr$$module$static$js$services$idb_mgr.exists(DATA_KEYS$$module$static$js$services$data_keys.KEY_KNBASE))alert("Base di conoscenza Vuota;");
else if(!await idbMgr$$module$static$js$services$idb_mgr.exists(DATA_KEYS$$module$static$js$services$data_keys.KEY_CONTEXT)||await confirm(`Vuoi creare un nuovo contesto per la query \n ${a}`))try{Spinner$$module$static$js$app_ui.show();AppMgr$$module$static$js$app_mgr.initApp();let b=await ragEngine$$module$static$js$rag_engine.buildContext(a);b||=[];const c=messages2html$$module$static$js$history_utils(b);setResponseHtml$$module$static$js$app_ui(c);TextInput$$module$static$js$app_ui.inp.value=""}catch(b){if(console.error("Error send1",
b),b&&"CancellationError"===b.type&&499===b.code)alert("Richiesta interrotta");else{const c=errorDumps$$module$static$js$app_ui(b);alert(c)}}finally{Spinner$$module$static$js$app_ui.hide()}},async send2(){if(this.inp.value){Spinner$$module$static$js$app_ui.show();var a=this.inp.value.trim();try{const b=await ragEngine$$module$static$js$rag_engine.runConversation(a),c=messages2html$$module$static$js$history_utils(b);""!=c&&(setResponseHtml$$module$static$js$app_ui(c),TextInput$$module$static$js$app_ui.inp.value=
"")}catch(b){if(console.error("Error send2",b),b&&"CancellationError"===b.type&&499===b.code)alert("Richiesta interrotta");else{const c=errorDumps$$module$static$js$app_ui(b);alert(c)}}finally{Spinner$$module$static$js$app_ui.hide()}}else alert("Ricorda di scrivere la Query  ")},clear(){this.inp.value="";this.inp.focus()}},TextOutput$$module$static$js$app_ui={init(){this.copyBtn=document.querySelector(".copy-output")},openWnd(){showThread$$module$static$js$app_ui()},async copy(){const a=document.querySelector("#id-text-out .div-text");
let b=a.textContent;if(!(2>b.trim().length)){a.classList.add("copied");this.copyBtn.classList.add("copied");try{b=textFormatter$$module$static$js$history_utils(b),await navigator.clipboard.writeText(b)}catch(c){console.error("Errore  ",c)}setTimeout(()=>{this.copyBtn.classList.remove("copied");a.classList.remove("copied")},5E3)}},clear(){document.querySelector("#id-text-out .div-text").textContent=""},async clearHistory(){await confirm("Confermi nuova conversazione? ")&&(await idbMgr$$module$static$js$services$idb_mgr.delete(DATA_KEYS$$module$static$js$services$data_keys.KEY_THREAD),
setResponseHtml$$module$static$js$app_ui(""))}},getTheme$$module$static$js$app_ui=()=>{"light"===UaDb$$module$static$js$services$uadb.read(DATA_KEYS$$module$static$js$services$data_keys.KEY_THEME)?document.body.classList.add("theme-light"):document.body.classList.add("theme-dark")},setTheme$$module$static$js$app_ui=a=>{"light"===a?(document.body.classList.remove("theme-dark"),document.body.classList.add("theme-light"),UaDb$$module$static$js$services$uadb.save(DATA_KEYS$$module$static$js$services$data_keys.KEY_THEME,
"light")):(document.body.classList.remove("theme-light"),document.body.classList.add("theme-dark"),UaDb$$module$static$js$services$uadb.save(DATA_KEYS$$module$static$js$services$data_keys.KEY_THEME,"dark"))},showReadme$$module$static$js$app_ui=()=>{wnds$$module$static$js$app_ui.wdiv.show(help1_html$$module$static$js$services$help)},showQuickstart$$module$static$js$app_ui=()=>{wnds$$module$static$js$app_ui.wdiv.show(help2_html$$module$static$js$services$help)},showQuery$$module$static$js$app_ui=()=>
{const a=UaDb$$module$static$js$services$uadb.read(DATA_KEYS$$module$static$js$services$data_keys.KEY_QUERY);a&&wnds$$module$static$js$app_ui.winfo.show(a)},showContextResponse$$module$static$js$app_ui=()=>{const a=UaDb$$module$static$js$services$uadb.read(DATA_KEYS$$module$static$js$services$data_keys.KEY_RESPONSE);a&&wnds$$module$static$js$app_ui.wpre.show(a)},showThread$$module$static$js$app_ui=async()=>{var a=await idbMgr$$module$static$js$services$idb_mgr.read(DATA_KEYS$$module$static$js$services$data_keys.KEY_THREAD);
...
module$static$js$app_ui.bindEventListener=bindEventListener$$module$static$js$app_ui;module$static$js$app_ui.getTheme=getTheme$$module$static$js$app_ui;module$static$js$app_ui.showHtmlThread=showHtmlThread$$module$static$js$app_ui;module$static$js$app_ui.wnds=wnds$$module$static$js$app_ui;const getClientIp$$module$static$js$services$logger=async()=>{try{const a=await fetch("https://api.ipify.org?format=json");return a.ok?(await a.json()).ip:"xxx.xxx.xxx.xxx"}catch(a){return console.error("Errore nel recuperare l'IP:",a),"err.xxx.xxx.xxx"}},sendLog$$module$static$js$services$logger=async(a,b={})=>{var c=window.location.hostname;if("127.0.0.1"===c||"localhost"===c)return console.log("Registrazione disattivata in ambiente locale"),!0;c=await getClientIp$$module$static$js$services$logger();
a={clientId:a,timestamp:(new Date).toISOString(),ip:c,userAgent:navigator.userAgent,...b};try{return(await fetch("https://simple-moniotr-default-rtdb.europe-west1.firebasedatabase.app/visite.json",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).ok}catch(d){return console.error("Errore durante l'invio del log:",d),!1}};var module$static$js$services$logger={sendLog:sendLog$$module$static$js$services$logger};const DialogManager$$module$static$js$services$uadialog={escapeHtml(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")},createDialog(a,b,c=""){const d=document.createElement("div"),e=document.createElement("div");d.className=`${a}-dialog`;d.classList.add("inv");e.className="overlay";c="prompt"===a?`<input type="text" class="prompt-input" value="${this.escapeHtml(c)}">`:"";b=this.escapeHtml(b);d.innerHTML=`
